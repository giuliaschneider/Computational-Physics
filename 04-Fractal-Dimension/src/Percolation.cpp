...
void Percolation::createLattice(double p, int marker){
  ...
  */
  ...
  //sprintf(filename,"python_scripts/lat_N%d_original.ppm",N);
  //saveFigure(filename);
}

int* Percolation::getLattice(){
  /**
      @brief: Return the percolation lattice
      @return lattice
  */
  return lat;
}
...
void Percolation::calcCSD_different_p(){
  ...
  */
  ...
  for(int i = 0;i<10;i++){
    ...
    //save::save_to_text(&nClusters,filename);
...
void Percolation::find_largestCluster(){
  /**
      @brief: finds the largest cluster and sets all its sites to 1 and all other sites to zero
      @return void
  */
  ...
  int iMax = distance(M.begin(),max_element(M.begin(), M.end()));

  // get all k-values belonging to the biggest cluster
  vector<int> iVec;
  int ktemp = 0;
  iVec.push_back(iMax);
  ...
  for(int i=0;i<M.size();i++){
    if(M[i]<0){
      ...
      if(ktemp == iMax){iVec.push_back(i);}
    }
  }
  //save::printLattice(lat,N);
  cout << "iMax = " << iMax << endl;
  //save::printVector(&iVec);
  #pragma omp parallel for
  for(int i=0;i<(N*N);i++){
    if(find(iVec.begin(),iVec.end(),lat[i])!=iVec.end()){lat[i]=1;}
    else {lat[i]=0;}
  }
  //save::printLattice(lat,N);
  sprintf(filename,"python_scripts/lat_N%d.ppm",N);
  //saveFigure(filename);
  cout << "Set largest Cluster" << endl;
}
