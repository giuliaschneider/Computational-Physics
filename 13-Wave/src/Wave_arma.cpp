...
void Wave::calcWave(){
  for(int t=0; t<nSteps+1; t++){
    ...
    */
    ...
    if((tend-t0)/nSteps*t == static_cast<int>((tend-t0)/nSteps*t) and int((tend-t0)/nSteps*t)%2 ==0 ){
      cout << "Saving " << (tend-t0)/nSteps*t << endl;
      sprintf(filename,"python_scripts/u_sin_b_%d_t%d.txt", int(100*b), int((tend-t0)/nSteps*t));
      //save::printVector(u_now, h);(tend-t0)/nSteps*t

      //save::save_to_text(temp, h, filename);
      u_now.save(filename, arma::raw_ascii);
    }
