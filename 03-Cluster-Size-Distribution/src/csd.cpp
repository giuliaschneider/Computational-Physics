  ...
  */
  ...
  if(lat->getValue(position)==1){
    ...
    if(position == 0){ // check if site is upper left corner
      lat->setValue(position, kmax); M.push_back(1);
    }
    else if(northValue==0){//no northern neighbor or north unoccupied
...
void csd::calcCSD_different_p(){
  ...
  */
  ...
  for(int i = 0;i<10;i++){
    ...
    sprintf(filename,"python_scripts/lat_N%d_p%.2f.ppm",L,p);
    //saveFigure(filename);
